...
public class SimpleUsageTest {
    ...
    @Test
    @DisplayName("superAdmin注册Operate-可以部署合约CredenceTPL.deploy的操作")
    @Order(2)
    void signUpOperate() throws InterruptedException, IOException {

        // superAdmin注册部署合约credence-net:CredenceTPL.deploy的操作，向<身份链>提交
    ...
    @Test
    @DisplayName("superAdmin授权给credence-net:usr-0可部署CredenceTPL的权限")
    @Order(3)
    void grantOperate() throws InterruptedException, IOException {
        ...
        Peer.Authorize authorize_1 = Peer.Authorize.newBuilder()
                ...
                .addOpId(DigestUtils.sha256Hex("credence-net:CredenceTPL.deploy"))  // CredenceTPL，需要改一下
    ...
    @Test
    @DisplayName("usr-0，具有了向业务链部署CredenceTPL的权限，因此向业务链部署合约")
    @Order(4)
    ...
    @Disabled
    @DisplayName("使用中间件，调用业务链中刚部署的合约，存证数据，使用http")
    @Order(7)
    void testCredenceProof_Connector_http() throws Exception {
        CloseableHttpClient httpClient = HttpClients.custom().build();
        RequestConfig config = RequestConfig.custom().setSocketTimeout(10000).setConnectTimeout(10000).build();
        HttpPost httpPost = new HttpPost(URI.create("http://localhost:8081/connector/upChain"));
        httpPost.setConfig(config);
        UpChainInfo upChainInfo = new UpChainInfo("credence-tpl-proof-uuid-0", "credence-net",
                "CredenceTPL", "1", "",
                "creProof3", "{\"uuid\":\"credence-tpl-proof-uuid-0\", \"data\" : \"{\\\"data1\\\":\\\"xyb002\\\",\\\"data2\\\":\\\"xyb003\\\"}\"}"
        );
        StringEntity entity = new StringEntity(JSON.toJSONString(upChainInfo), "utf-8");
    ...
    @Disabled
    @DisplayName("使用中间件，调用业务链中刚部署的合约，存证数据，使用https，双向认证")
    @Order(8)
    void testCredenceProof_Connector_https() throws Exception {
        ...
        RequestConfig config = RequestConfig.custom().setSocketTimeout(10000).setConnectTimeout(10000).build();
        // https请求
        HttpPost httpsPost = new HttpPost(URI.create("https://localhost:8081/connector/upChain"));
        httpsPost.setConfig(config);
        UpChainInfo upChainInfo = new UpChainInfo("credence-tpl-proof-uuid-2", "credence-net",
                "CredenceTPL", "1", "",
                "creProof3", "{\"uuid\":\"credence-tpl-proof-uuid-2\", \"data\" : \"{\\\"data1\\\":\\\"xyb002\\\",\\\"data2\\\":\\\"xyb003\\\"}\"}"
        );
        StringEntity entity = new StringEntity(JSON.toJSONString(upChainInfo), "utf-8");
        httpsPost.setEntity(entity);
    ...
    @Disabled
    @DisplayName("使用中间件，查询")
    @Order(9)
    void testCredenceProof_Connector_query() throws Exception {
        SSLContext sslContext = SSLContexts.custom()
                .loadTrustMaterial(new File("jks/jdk13/121000005l35120456.node1.jks"), "123".toCharArray(), new TrustSelfSignedStrategy())
                .loadKeyMaterial(new File("jks/jdk13/121000005l35120456.node1.jks"), "123".toCharArray(), "123".toCharArray())
                .build();
        ...
        RequestConfig config = RequestConfig.custom().setSocketTimeout(10000).setConnectTimeout(10000).build();
        // https请求
        String bid = "12345"; // 自己填
        HttpGet httpGet = new HttpGet(URI.create("https://localhost:8080/connector/queryDataStatus/" + bid));
    ...
    static class UpChainInfo {
        String bid;  //uuid
        String networkingName; // identity-net
        String contractName; // Contract A
        String contractVersion; // 1
        String contractInstanceId; // ""
        String contractMethodName; // transfer
        String contractParameters; // {from to amount}

        public UpChainInfo(String bid, String networkingName, String contractName, String contractVersion, String contractInstanceId, String contractMethodName, String contractParameters) {
            this.bid = bid;
        ...
        public String getNetworkingName() {
            return networkingName;
        }

        public void setNetworkingName(String networkingName) {
            this.networkingName = networkingName;
        }
        ...
        public void setContractParameters(String contractParameters) {
            this.contractParameters = contractParameters;
        }
    }
