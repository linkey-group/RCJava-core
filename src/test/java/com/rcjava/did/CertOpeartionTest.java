...
public class CertOpeartionTest extends DidTest {
    ...
    void testSignUpAllTypeCert_3() throws IOException, InterruptedException {
        ...
        Peer.Authorize authorize = Peer.Authorize.newBuilder()
                ...
                .build();
        String tranId = UUID.randomUUID().toString();
        Peer.Transaction tran = node1Creator.createInvokeTran(tranId, node1CertId, chaincodeId, grantOperate,
    ...
    void testSignUpAllTypeCert_4() throws IOException, InterruptedException {
        ...
        TranCreator usr0_tranCreator_1 = genUsrTranCreator(user0_creditCode, user0_cert_1);
        Peer.CertId usr0_certId_1 = Peer.CertId.newBuilder().setCreditCode(user0_creditCode).setCertName(user0_cert_1).build();
        Peer.Transaction tran_1 = usr0_tranCreator_1.createInvokeTran(tranId_1, usr0_certId_1, chaincodeId, grantOperate,
                JSONObject.toJSONString(Collections.singletonList(JsonFormat.printer().print(authorize))), 0, "");
        postClient.postSignedTran(tran_1);
        TimeUnit.SECONDS.sleep(2);
        Peer.TransactionResult tranResult_1 = infoClient.getBlockByHeight(infoClient.getChainInfo().getHeight()).getTransactionResults(0);
