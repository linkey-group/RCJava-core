...
public class Did4UnderlineTest {
    ...
    protected static String user0_creditCode_did = "identity-net:usr_0";
    protected static String user1_creditCode_did = "identity-net:usr_1";
    protected static String user2_creditCode_did = "identity-net:usr_2";

    protected static String user0_creditCode_cre = "credence-net:usr_0";
    protected static String user1_creditCode_cre = "credence-net:usr_1";
    protected static String user2_creditCode_cre = "credence-net:usr_2";
...
//    Did4UnderlineTest.SignerCert usr0_signer_cert_1 = genCertSigner(user0_creditCode, user0_pem_1, user0_cert_1);
//    Did4UnderlineTest.SignerCert usr1_signer_cert_1 = genCertSigner(user1_creditCode, user1_pem_1, user1_cert_1);

    Did4UnderlineTest.SignerCert usr0_signer_cert = getCertSigner(user0_creditCode_did, user0_cert_0);
    Did4UnderlineTest.SignerCert usr1_signer_cert = getCertSigner(user1_creditCode_did, user1_cert_0);
    Did4UnderlineTest.SignerCert usr2_signer_cert = getCertSigner(user2_creditCode_did, user2_cert_0);
    ...
    public Did4UnderlineTest() throws IOException {
    }
...
//        genCertPriKeyJks();
    ...
    static void genCertPriKeyJks() throws Exception {
        for (int i = 0; i <= 3; i++) {
            for (int j = 0; j < 2; j++) {
                ...
                CertUtil.genJksFile(new File("jks/test/did4Underline/" + usr_alias + ".jks"), usr_alias, "123");
            }
        }
        for (int i = 0; i <= 3; i++) {
            for (int j = 0; j < 2; j++) {
                ...
                PemUtil.exportToPemFile(new File("jks/test/did4Underline/" + usr_alias + ".cer"), certPrivateKey.getCertificate());
                PemUtil.exportToPemFile(new File("jks/test/did4Underline/" + usr_alias + ".pkcs8"), certPrivateKey.getPrivateKey(), false);
            }
        }
    }
     ...
     * @throws Exception
     */
    protected Did4UnderlineTest.SignerCert getCertSigner(String CreditCode, String certName) throws IOException {
        // 证书pem字符串
        String certPem = new String(Files.readAllBytes(new File(String.format("jks/test/did4Underline/%s_%s.cer", CreditCode.split(":")[1], user0_cert_0)).toPath()));
        ...
        Peer.Signer signer = Peer.Signer.newBuilder()
                ...
                .setSignerValid(true)
                .build();
        return new Did4UnderlineTest.SignerCert(signer, cert_proto);
    }
    ...
    protected Peer.Certificate getUsrCert(String creditCode, String certName) {
        X509Certificate x509Certificate = CertUtil.generateX509Cert(new File(String.format("jks/test/did4Underline/%s_%s.cer", creditCode.split(":")[1], certName)));
        try {
            String certPem = PemUtil.toPemString(x509Certificate);
            return Peer.Certificate.newBuilder()
                    .setCertificate(certPem)
    ...
    protected TranCreator genUsrTranCreator(String netWorkId, String creditCode, String certName) {
        try {
            PrivateKey privateKey = KeyUtil.generatePrivateKey(
                    new PEMParser(new FileReader(String.format("jks/test/did4Underline/%s_%s.pkcs8", creditCode.split(":")[1], certName))),
                    "123");
